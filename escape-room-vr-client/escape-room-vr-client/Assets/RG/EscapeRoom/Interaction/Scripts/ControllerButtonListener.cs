...
namespace RG.EscapeRoom.Interaction.Scripts
{
    ...
    {
        public Dictionary<IControllerButtonData.Controller, HashSet<IControllerButtonData.Button>> pressedButtons = new Dictionary<IControllerButtonData.Controller, HashSet<IControllerButtonData.Button>>();
        ...
        public void NotifyButtonPressed(IControllerButtonData.Controller controller, IControllerButtonData.Button button)
        {
            GetControllersPressedButtons(controller).Add(button);
        }
        
        public void NotifyButtonReleased(IControllerButtonData.Controller controller, IControllerButtonData.Button button)
        {
            GetControllersPressedButtons(controller).Remove(button);
        }

        private HashSet<IControllerButtonData.Button> GetControllersPressedButtons(IControllerButtonData.Controller controller)
        {
    ...
    public class ControllerButtonListener: ITickable
    {
        ...
        public void Tick()
        {
            for (int i = 0; i < leftHandInputDevices.Count; i++)
            {
                ReadButton(leftHandInputDevices[i], CommonUsages.triggerButton, IControllerButtonData.Controller.Left, IControllerButtonData.Button.Trigger);
                ReadButton(leftHandInputDevices[i], CommonUsages.gripButton, IControllerButtonData.Controller.Left, IControllerButtonData.Button.Grip);
            }
            for (int i = 0; i < rightHandInputDevices.Count; i++)
            {
                ReadButton(rightHandInputDevices[i], CommonUsages.triggerButton, IControllerButtonData.Controller.Right, IControllerButtonData.Button.Trigger);
                ReadButton(rightHandInputDevices[i], CommonUsages.gripButton, IControllerButtonData.Controller.Right, IControllerButtonData.Button.Grip);
            }
        }

        private void ReadButton(InputDevice device, InputFeatureUsage<bool> xrButtonId, IControllerButtonData.Controller controller, IControllerButtonData.Button internalButtonId)
        {
            bool buttonValue;
